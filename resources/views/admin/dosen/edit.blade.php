<x-app-layout>
    <x-slot name="header">Edit Dosen</x-slot>
    ...
    <div class="max-w-lg">
        <div style="background-color: #0F1C2E; border: 1px solid #1e2d42;" class="rounded-lg p-8">
            ...
            <h3 class="text-white text-xl mb-6" style="font-family: 'Playfair Display', serif;">Edit Data Dosen</h3>

            <form action="{{ route('admin.dosen.update', $dosen) }}" method="POST">
                @csrf @method('PUT')
                <div class="mb-5">
                    <label class="block text-xs uppercase tracking-widest text-gray-400 mb-2">NIDN</label>
                    ...
                    @error('nidn') <p class="text-red-400 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div class="mb-8">
                ...
                <div class="flex gap-3">
                    <button type="submit"
                        ...
                        class="text-white px-6 py-2.5 rounded text-sm font-medium hover:opacity-90 transition">
                        Update
                    </button>
                    <a href="{{ route('admin.dosen.index') }}"
                        style="border: 1px solid #2a3a50; color: #6b7280;"
                        class="px-6 py-2.5 rounded text-sm hover:border-gray-500 transition">
                        Batal
                    </a>
                </div>
            </form>
