<x-app-layout>
    ...
    <div style="background-color: #0F1C2E; border: 1px solid #1e2d42;" class="rounded-lg p-6">
        ...
        @if(session('success'))
            <div style="border: 1px solid #C9A84C; color: #C9A84C; background-color: #1a2a1a;" class="px-4 py-3 rounded mb-5 text-sm">
                {{ session('success') }}
            </div>
        @endif

        <div class="flex flex-wrap justify-between items-center gap-3 mb-6">
            <form method="GET" action="{{ route('admin.dosen.index') }}" class="flex gap-2">
                ...
                <a href="{{ route('admin.dosen.index') }}"
                    class="text-gray-500 px-3 py-2 text-sm hover:text-gray-300 transition">
                    Reset
                </a>
                @endif
            </form>
            <a href="{{ route('admin.dosen.create') }}"
                ...
                class="text-white px-5 py-2 rounded text-sm font-medium hover:opacity-90 transition tracking-wide">
                + Tambah Dosen
            </a>
        </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr style="border-bottom: 1px solid #C9A84C;">
                    ...
                    <th class="text-left text-gray-400 text-xs uppercase tracking-widest pb-3 px-2">NIDN</th>
            ...
            <tbody>
                ...
                <tr style="border-bottom: 1px solid #162032;" class="hover:bg-white hover:bg-opacity-5 transition">
                    ...
                    <td class="py-4 px-2 font-mono text-xs" style="color: #C9A84C;">{{ $dosen->nidn }}</td>
                    <td class="text-gray-200 py-4 px-2 font-medium">{{ $dosen->nama }}</td>
                    <td class="py-4 px-2 text-center">
                        <a href="{{ route('admin.dosen.edit', $dosen) }}"
                            class="text-gray-400 hover:text-yellow-500 text-xs uppercase tracking-wide mr-3 transition">
                            Edit
                        </a>
                        <form action="{{ route('admin.dosen.destroy', $dosen) }}" method="POST" class="inline"
                            onsubmit="return confirm('Yakin hapus?')">
                            @csrf @method('DELETE')
                            <button class="text-gray-600 hover:text-red-400 text-xs uppercase tracking-wide transition">
                                Hapus
                            </button>
                        </form>
                    </td>
                ...
                <tr>
                    <td colspan="4" class="text-center py-12 text-gray-600">
                        ...
                        {{ request('search') ? 'Data tidak ditemukan' : 'Belum ada data dosen' }}
                    </td>
                </tr>
                @endforelse
            </tbody>
        ...
        <div class="mt-6">{{ $dosens->links() }}</div>
    </div>
